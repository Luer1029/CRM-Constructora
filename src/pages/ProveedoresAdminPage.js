...
function ProveedoresAdminPage() {
  ...
  const promedioGlobalCalificaciones = useMemo(() => {
    ...
    const suma = proveedores.reduce((acumulado, proveedor) => {
      // Generar dinámicamente para el cálculo del promedio global
      const proyectosTerminadosGenerados = generarProyectosTerminados(proveedor.id);
      const resenasGeneradas = generarResenas(proveedor.id, proyectosTerminadosGenerados);
      const calificacionPromedio = resenasGeneradas.length > 0 
        ? resenasGeneradas.reduce((sum, r) => sum + r.calificacion, 0) / resenasGeneradas.length
        : 0;
      return acumulado + calificacionPromedio;
    }, 0);
  ...
  const totalReseniasGlobal = useMemo(() => {
    return proveedores.reduce((acumulado, proveedor) => {
      // Generar dinámicamente para el cálculo total global
      const proyectosTerminadosGenerados = generarProyectosTerminados(proveedor.id);
      const resenasGeneradas = generarResenas(proveedor.id, proyectosTerminadosGenerados);
      return acumulado + resenasGeneradas.length;
    }, 0);
  }, [proveedores]);

  const proveedoresFiltrados = useMemo(() => {
    let resultado = proveedores.map((proveedor) => {
      // Generar dinámicamente los proyectos terminados y reseñas ficticias para sincronizar con ProveedorPage
      const proyectosTerminadosGenerados = generarProyectosTerminados(proveedor.id);
      const resenasGeneradas = generarResenas(proveedor.id, proyectosTerminadosGenerados);
      
      // Calcular promedio y cantidad desde las reseñas generadas
      const calificacionPromedio = resenasGeneradas.length > 0 
        ? Number((resenasGeneradas.reduce((sum, r) => sum + r.calificacion, 0) / resenasGeneradas.length).toFixed(1))
        : 0;
      const totalCalificaciones = resenasGeneradas.length;
