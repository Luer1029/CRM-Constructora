...
function ProveedorPage({ onLogout }) {
  ...
  const [infoUsuario, setInfoUsuario] = useState({ nombre: "", apellido: "", correo: "", clasificacion: "" });
  ...
  useEffect(() => {
    const cargarConfiguracionProveedor = async () => {
      ...
      try {
        ...
        const resenasGeneradas = generarResenas(usuarioActual.uid, proyectosTerminadosGenerados);
        ...
        if (usuarioSnap.exists()) {
          ...
          // Guardar información del usuario para la bienvenida
          setInfoUsuario({
            nombre: datos?.nombre || "",
            apellido: datos?.apellido || "",
            correo: datos?.correo || usuarioActual?.email || "",
            clasificacion: datos?.clasificacionProveedor || "basica"
          });
        }
  ...
  return (
    <div className="pagina-rol">
      ...
      <div className="contenedor-rol proveedor-view">
        ...
        <div className="lista-contenedor" style={{ marginBottom: "24px", backgroundColor: "#f0f9ff", borderLeft: "4px solid #0284c7" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <h2 style={{ margin: "0 0 8px 0", color: "#0c4a6e" }}>
                👋 Bienvenido, {infoUsuario.nombre || "Proveedor"}
              </h2>
              <p style={{ margin: "0 0 8px 0", color: "#0f766e", fontSize: "14px" }}>
                {infoUsuario.apellido && `${infoUsuario.nombre} ${infoUsuario.apellido}`}
              </p>
              <div style={{ display: "flex", gap: "16px", fontSize: "13px", color: "#475569" }}>
                <span><strong>Email:</strong> {infoUsuario.correo || "No disponible"}</span>
                <span><strong>Clasificación:</strong> {infoUsuario.clasificacion?.charAt(0).toUpperCase() + infoUsuario.clasificacion?.slice(1) || "Básica"}</span>
              </div>
            </div>
